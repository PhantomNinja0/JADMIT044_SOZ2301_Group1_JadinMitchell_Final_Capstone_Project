...
const Preview = ({ podcastId, onListenClick, onFavoriteClick, onEpisodeComplete, onEpisodeProgress }) => {
  // State variables
      ...
      {Array.isArray(podcast.seasons) ? (
        podcast.seasons.map((season, index) => (
          <div key={index}>
            ...
            <button className='seasonbtn' onClick={() => handleSeasonClick(season)}>
              <h2>Season: {season.season}</h2>
              <img src={season.image} className="show-image" alt={season.title} />
              <h3>Click to see all episodes for Season {season.season}</h3>
            </button>
            
            {/* List of episodes for the selected season */}
            <div className={`episodes-list ${selectedSeason === season ? 'show-episodes' : ''}`}>
            <ul className='single-ep'>
              {season.episodes.map((episode, episodeIndex) => (
                <li key={episodeIndex}>
                  ...
                  {/* Favorite button */}
                  <button className='favebtn' onClick={handleFavoriteClick}>Favorite</button>
                </li>
              ))}
            </ul>
            </div>
